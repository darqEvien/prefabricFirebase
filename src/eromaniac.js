  ...
  subcategories.forEach(([subCategoryName, subCategory]) => { 
    ...
    let sortedItems = subCategory.documents
      .filter(item => 
        ...
        item.tag && 
        selectedCategories.some(tag => item.tag.includes(tag)) // En az bir tag'in selectedCategories içinde olup olmadığını kontrol et
      )
...
function updateItemSizesOnSelection() {
  Object.keys(categoriesData).forEach((categoryName) => {
    ...
    if (category.priceFormat === "artis") {
      ...
      let selectedItemFark = 0;

      items.forEach((item) => {
        const itemButton = item.querySelector(".proButs");
        const alanPrice =
          parseFloat(itemButton.getAttribute("data-alan-price")) || 0;
        const price = parseFloat(itemButton.getAttribute("data-price")) || 0;
        const isSelected = itemButton.classList.contains("selected");

        if (isSelected) {
          // Seçili item için fark hesaplama
          selectedItemFark = alan * alanPrice + cevre * price;
      ...
      items.forEach((item) => {
        ...
        if (!isSelected) {
          if (alanPrice > 0) {
            ...
            item.querySelector(
              ".sizes__paragh"
            ).innerHTML = `Fark:<br><span style="color:green;">+${fark.toLocaleString(
              "tr-TR"
            )}₺</span>`;
          } else {
            const fark = alan * alanPrice + cevre * price;
...
function handleItemClick(button, categoriesData) {
  ...
  requestAnimationFrame(() => {
    ...
    const subcategories = Object.keys(categoriesData).filter(cat => 
      categoriesData[cat].parentCategory === categoryName
    );
    
    subcategories.forEach(subCategoryName => {
      renderSubcategoriesRecursively(subCategoryName, categoriesData);
    });
...
function showDetails(item) {
  ...
  document.getElementById(
    "modal-dimensions"
  ).textContent = `Boyut: ${item.width} x ${item.height} m`;
